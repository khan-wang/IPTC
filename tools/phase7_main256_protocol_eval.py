...
def build_standardized_subset_with_mask_mode(
    *,
    ...
    cursor = 0
    sample_index = 0
    for spec in phase5a.MASK_BUCKET_SPECS:
        bucket = spec["bucket"]
...
def write_dataset_scope_md(
    *,
    ...
    lines = [
        ...
        f"- Bucket mask catalog counts: `10-20={mask_catalog_counts['10-20']}, 20-30={mask_catalog_counts['20-30']}, 30-40={mask_catalog_counts['30-40']}, 40-50={mask_catalog_counts['40-50']}, 50-60={mask_catalog_counts['50-60']}`",
        "",
...
def attach_group_fid(
    ...
    *,
    ...
    for row in summary_rows:
        group = "10-60" if group_key == "dataset" else row[group_key]
...
def build_pair_rows(
    ...
    for pair_spec in PAIRWISE_COMPARISONS:
        ...
        if not pair_group_rows:
            continue
        for group in ("20-40", "40-60", "10-60"):
            if group == "10-60":
...
def main() -> None:
    ...
    bucket_counts = {bucket: samples_per_bucket for bucket in BUCKET_ORDER}
    ...
    config = {
        "phase": f"Phase 7 generic 256 protocol evaluation: {dataset_name}",
        ...
        "fid_policy": (
            ...
            if args.skip_fid
            else "computed_by_mask_group_and_10_60"
        ),
    }
    ...
    for row in per_image_metrics:
        ...
        for group in phase7_places2.group_for_sample(sample):
            expanded = dict(row)
    ...
    (output_dir / "summary_main_table_latex.txt").write_text(phase7_places2.latex_from_rows(summary_main), encoding="utf-8")
    ...
    main_command = [
        ...
        sys.executable,
        "tools/phase7_main256_protocol_eval.py",
        f"--put-root {put_root}",
    ...
    readme_lines = [
        f"# Phase 7 Generic 256 Protocol Evaluation: {dataset_name}",
        "",
        f"{dataset_name} 256x256 protocol evaluation for the selected PUT-family methods.",
        "",
        f"- Actual image count: `{len(sample_records)}`",
    ...
    (output_dir / "phase7_main256_protocol_summary.json").write_text(
